...
export function PngToJpgTool() {
  ...
  const handleFiles = async (files: File[]) => {
    ...
    const err = validateFile(f, ['image/png'], 50);
    if (err) { setError(err); return; }
    try {
      const url = await fileToDataURL(f);
      setFile(f); setPreview(url);
      track('tool_start', { tool_id: 'png-to-jpg' });
  ...
  const convert = async () => {
    ...
    try {
      ...
      track('tool_complete', { tool_id: 'png-to-jpg' });
  ...
  return (
    <div className="space-y-4">
      ...
      {!file ? (
        <Dropzone onFiles={handleFiles} accept="image/png,.png" maxSizeMB={50} />
      ) : (
        <>
          <div className="flex flex-col gap-4 sm:flex-row">
            <div className="rounded-xl border border-slate-200 p-2 dark:border-surface-dark-border" style={{ backgroundImage: 'linear-gradient(45deg, #ddd 25%, transparent 25%), linear-gradient(-45deg, #ddd 25%, transparent 25%), linear-gradient(45deg, transparent 75%, #ddd 75%), linear-gradient(-45deg, transparent 75%, #ddd 75%)', backgroundSize: '16px 16px', backgroundPosition: '0 0, 0 8px, 8px -8px, -8px 0' }}>
