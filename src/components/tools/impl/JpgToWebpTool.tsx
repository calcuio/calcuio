...
export function JpgToWebpTool() {
  ...
  const handleFiles = async (files: File[]) => {
    ...
    const err = validateFile(f, ['image/jpeg', 'image/jpg'], 50);
    if (err) { setError(err); return; }
    try { const url = await fileToDataURL(f); setFile(f); setPreview(url); track('tool_start', { tool_id: 'jpg-to-webp' }); }
  ...
  const convert = async () => {
    ...
    try {
      ...
      const blob = await new Promise<Blob>((resolve, reject) => {
        canvas.toBlob((b) => b ? resolve(b) : reject(new Error('Conversion failed')), 'image/webp', quality / 100);
      });
      setProgress(100);
      setResult({ blob, size: blob.size });
      track('tool_complete', { tool_id: 'jpg-to-webp' });
  ...
  if (result && file) {
    return <SuccessState originalSize={file.size} newSize={result.size} onReset={reset} downloadButton={<DownloadButton blob={result.blob} filename={`${getBaseName(file.name)}.webp`} />} />;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {!file ? (
        <Dropzone onFiles={handleFiles} accept=".jpg,.jpeg,image/jpeg" maxSizeMB={50} />
      ) : (
        <>
